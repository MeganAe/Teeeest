...
var AuditInterceptor = function () {
    ...
    var AuditInterceptor = _classThis = /** @class */ (function () {
        function AuditInterceptor_1(auditService, reflector) {
        ...
        AuditInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var user = request.user, method = request.method, url = request.url, ip = request.ip, headers = request.headers;
            var auditConfig = this.reflector.get(audit_decorator_1.AUDIT_KEY, context.getHandler());
            if (!auditConfig || !user) {
                return next.handle();
            }
            ...
            return next.handle().pipe((0, operators_1.tap)({
                next: function (data) {
                    _this.auditService.log({
                        userId: user.id,
                        action: "".concat(method, " ").concat(url),
