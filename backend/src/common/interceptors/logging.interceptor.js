...
var LoggingInterceptor = function () {
    ...
    var LoggingInterceptor = _classThis = /** @class */ (function () {
        ...
        LoggingInterceptor_1.prototype.intercept = function (context, next) {
            ...
            var method = request.method, url = request.url, ip = request.ip;
            ...
            return next.handle().pipe((0, operators_1.tap)({
                next: function () {
                    ...
                    _this.logger.log("".concat(method, " ").concat(url, " ").concat(response.statusCode, " - ").concat(duration, "ms - ").concat(userAgent, " ").concat(ip));
                },
                error: function (error) {
                    var duration = Date.now() - startTime;
                    _this.logger.error("".concat(method, " ").concat(url, " ").concat(error.status, " - ").concat(duration, "ms - ").concat(userAgent, " ").concat(ip));
                },
            }));
        };
