...
var RateLimitGuard = function () {
    ...
    var RateLimitGuard = _classThis = /** @class */ (function () {
        function RateLimitGuard_1(reflector) {
            this.reflector = reflector;
            this.requests = new Map();
        }
        RateLimitGuard_1.prototype.canActivate = function (context) {
            var request = context.switchToHttp().getRequest();
            var key = request.ip || request.connection.remoteAddress;
            var now = Date.now();
            var windowMs = 60000; // 1 minute
            var maxRequests = 100;
            var record = this.requests.get(key);
            if (record) {
                ...
                if (record.count >= maxRequests) {
                    throw new common_1.HttpException('Trop de requêtes', common_1.HttpStatus.TOO_MANY_REQUESTS);
                }
                record.count++;
